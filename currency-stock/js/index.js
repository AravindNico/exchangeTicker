...
var sendRequest = function(p_name,p_totalTime){
    pubnub.publish({
        channel : 'exchangedata',
        message : '{\"name\":\"' + p_name + '\",\"backtime\":' + p_totalTime + ',\"channel\":\"privateChannel\"}'
    });
}
...
var updatePrice = function(p_message){
	if(counterDisplayState[p_message.name] == true){
        ...
        //If there is a connection error with server, waits for the server to connect
			...
			try{
				sendRequest(p_message.name,MAX_HISTORY_FETCH_LIMIT)
			}
			catch(err){
				console.log(err)
			}
...
var updateChart = function(p_message){
